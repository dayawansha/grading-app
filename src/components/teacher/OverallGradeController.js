...
function OverallGradeController() {
    ...
    useEffect(() => {
        if (isLoading) {
            if(selectedAssignment != null && selectedCourse != null){
                 ...
                 * API call
                 */
                axios.get(`${process.env.REACT_APP_BASE_URL}${process.env.REACT_APP_FUN_STUDENT_RESULT_GRADE}${1}${/assignment/}${selectedAssignment}${/course/}${selectedCourse}`)
                    .then(response => {
    ...
    return (
        <div>
            <h1>Overall grade details for assignments</h1>
            <hr />
            <Row>
                <div>
                    <Col lg={12} sm={6}>
...
export default OverallGradeController;